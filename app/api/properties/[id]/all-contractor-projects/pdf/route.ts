...
export async function GET(
    ...
    const backendUrl = `${API_URL}/api/properties/${id}/all-contractor-projects/pdf`;
    ...
    if ('errorResponse' in result) {
        console.log(`[PDF Route] Unauthorized or failed token refresh for all contractor projects PDF ${id} after ${Date.now() - routeStartTime}ms`);
    ...
    if (!backendResponse.ok) {
        ...
        console.log(`[PDF Route] Backend returned error status ${backendResponse.status} for all contractor projects PDF ${id} after ${Date.now() - routeStartTime}ms`);
    ...
    if (contentType.includes('application/json')) {
        ...
        if (data && data.downloadUrl) {
            ...
            console.log(`[PDF Route] Successfully proxying S3 stream for all contractor projects PDF ${id} in ${Date.now() - routeStartTime}ms`);
    ...
    console.log(`[PDF Route] Successfully proxying fallback PDF stream for all contractor projects PDF ${id} in ${Date.now() - routeStartTime}ms`);
