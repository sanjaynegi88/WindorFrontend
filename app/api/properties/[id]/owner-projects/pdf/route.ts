...
export async function GET(
    ...
    if ('errorResponse' in result) {
        console.log(`[PDF Route] Unauthorized or failed token refresh for property owner PDF ${id} after ${Date.now() - routeStartTime}ms`);
    ...
    if (!backendResponse.ok) {
        ...
        console.log(`[PDF Route] Backend returned error status ${backendResponse.status} for property owner PDF ${id} after ${Date.now() - routeStartTime}ms`);
    ...
    if (contentType.includes('application/json')) {
        ...
        if (data && data.downloadUrl) {
            ...
            console.log(`[PDF Route] Successfully proxying S3 stream for property owner PDF ${id} in ${Date.now() - routeStartTime}ms`);
    ...
    console.log(`[PDF Route] Successfully proxying fallback PDF stream for property owner PDF ${id} in ${Date.now() - routeStartTime}ms`);
