...
/**
 * Streaming proxy for multiple-property PDF report downloads.
 * Same approach as /api/reports/[id]/download — pipes the external API
 * response body directly to the browser so infra timeouts are not triggered.
 */
export async function GET(request: NextRequest) {
    const routeStartTime = Date.now();
    // Forward all query params (filters) to the external API
    ...
    if ('errorResponse' in result) {
        console.log(`[PDF Route] Unauthorized or failed token refresh for multiple reports after ${Date.now() - routeStartTime}ms`);
    ...
    if (!backendResponse.ok) {
        ...
        console.log(`[PDF Route] Backend returned error status ${backendResponse.status} for multiple reports after ${Date.now() - routeStartTime}ms`);
    ...
    if (contentType.includes('application/json')) {
        ...
        if (data && data.downloadUrl) {
            ...
            const secureContentDisposition = secureResponse.headers.get('content-disposition') ??
                'attachment; filename="top-10-properties-report.pdf"';

            console.log(`[PDF Route] Successfully proxying S3 stream for multiple reports in ${Date.now() - routeStartTime}ms`);
    ...
    console.log(`[PDF Route] Successfully proxying fallback PDF stream for multiple reports in ${Date.now() - routeStartTime}ms`);
