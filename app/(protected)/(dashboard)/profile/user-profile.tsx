  ...
  .superRefine((data, ctx) => {
    ...
    if (role === "insurance_company") {
      ...
      if (!data.cityOfficial?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "City Official is required",
          path: ["cityOfficial"],
        });
      }
      ...
      if (!data.cityPhone) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "City Phone is required",
          path: ["cityPhone"],
        });
      }
      if (!data.title?.trim()) {
...
export default function UserProfile() {
  ...
  const isSubAccount =
    contextUser?.user?.sub_account === true ||
    contextUser?.sub_account === true ||
  ...
  const hasActiveMembership = isSubAccount
    ? Boolean(
        ((user as any)?.current_subscription ??
          (user as any)?.user?.current_subscription) &&
        ((
          (user as any)?.current_subscription?.status ??
          (user as any)?.user?.current_subscription?.status
        )?.toUpperCase() === "ACTIVE" ||
          !(
            (user as any)?.current_subscription?.status ??
            (user as any)?.user?.current_subscription?.status
    ...
    async function fetchProfile() {
      try {
        ...
        setUser({
          ...
          company_name:
            data.company_name ?? data.form_details?.company_name ?? null,
        });
        ...
        form.reset({
          ...
          city_id: data.form_details?.city_id
            ? String(data.form_details.city_id)
            : data.user?.city_id
              ? String(data.user.city_id)
              : "",
          serviceTypes: data.form_details?.serviceTypes || [],
          // Property fields
          propertyAddress: data.form_details?.propertyAddress || "",
          ownerDateStart: data.form_details?.ownerDateStart
            ? data.form_details.ownerDateStart.split("T")[0]
            : "",
          ...
          company_name:
            data.company_name || data.form_details?.company_name || "",
        });
      } catch (error: any) {
  ...
  async function onSubmit(data: ProfileFormValues) {
    ...
    if (
      ...
      (!allowedRoleKeys || allowedRoleKeys.includes("serviceTypes"))
    ) {
    ...
    setUser({
      ...
      city_id:
        updatedData.user?.city_id ?? updatedData.form_details?.city_id ?? null,
    ...
    if (setContextUser) {
      setContextUser({
        ...
        company_name: updatedCompany,
        profile_image_url:
          updatedData.profile_image_url ?? contextUser?.profile_image_url,
      });
    }
  ...
  return (
    <Content className="p-4 md:p-6 lg:p-10 mx-auto">
        ...
        <div className="flex-1 w-full min-w-0 space-y-8">
                ...
                >
                  ...
                  <div className="px-4 md:px-8 py-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                    ...
                    <div className="md:col-span-2">
                      <Badge
                        variant="outline"
                        className="font-bold uppercase tracking-wider bg-primary/5 text-primary border-primary/10 px-3 py-1"
                      >
                        {toPascalCase(
                          user?.role ?? user?.roleEntity?.role_name ?? "Guest",
                        )}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <RoleForm
                    ...
                    isEditing={isEditing}
                    disabled={isSubAccount}
                    isStateDisabled={role === "city_inspector"}
                    isCityDisabled={role === "city_inspector"}
                    selectedStateId={selectedStateId}
                    onStateSelect={setSelectedStateId}
                    selectedCityName={selectedCityName}
                    onCitySelect={(_, name) => setSelectedCityName(name || "")}
                    isPresent={isPresent}
          ...
          <Card className="border shadow-lg rounded-2xl overflow-hidden bg-background">
            ...
            <CardContent className="p-0 divide-y">
              ...
              <div className="px-4 md:px-8 py-6 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="size-10 rounded-xl bg-red-50 flex items-center justify-center text-red-500 border border-red-100 shadow-sm">
                ...
                <Button
                  ...
                  size="sm"
                  className="rounded-xl font-bold text-[10px] uppercase tracking-wider h-9 px-4 text-red-500 border-red-200 hover:bg-red-50 hover:text-red-600"
                >
                  Sign Out
                </Button>
              </div>
              ...
              {showPurchasebtn && (
                <div className="px-4 md:px-8 py-6 flex flex-wrap items-center justify-between gap-4">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setPurchaseUsersOpen(true)}
                    size="sm"
                    className="rounded-xl font-bold text-[10px] uppercase tracking-wider h-9 px-4"
                  >
                    Purchase User
                      ...
                      <>
                        ...
                        <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-4 rounded-2xl border border-primary/15 relative overflow-hidden">
                          ...
                          <p className="text-[11px] text-muted-foreground mt-2 leading-snug">
                            Your monthly report inspection access and balance
                            are allocated by this membership tier.
                          </p>
                        </div>
                ...
                {user?.current_subscription?.status === "ACTIVE" ? (
                  <>
                    ...
                    <p className="text-sm text-muted-foreground font-medium mb-6 leading-relaxed max-w-[260px]">
                      {user.current_subscription?.plan?.description ||
                        "You have access to all the features and exclusive verification reports."}
                    </p>
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground font-medium mb-10 leading-relaxed max-w-[260px]">
                      Join thousands of professionals and unlock exclusive
                      verification reports and instant data exports.
                    </p>
                    <Button
                      onClick={() => router.push("/plans")}
                      className="w-full bg-primary hover:bg-primary/90 text-white rounded-2xl h-14 font-black transition-all hover:shadow-[0_10px_30px_rgba(59,130,246,0.3)] active:scale-95 text-xs uppercase tracking-[0.2em]"
                    >
                      Select Plan
                    </Button>
                  </>
      ...
      <ImageSourcePickerDialog
        ...
        onSelectCamera={() => {
          const input = cameraImageInputRef.current;
          if (input) {
            input.value = "";
            input.click();
          }
        ...
        onSelectGallery={() => {
          const input = galleryImageInputRef.current;
          if (input) {
            input.value = "";
            input.click();
          }
