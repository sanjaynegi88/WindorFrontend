    ...
    try {
      ...
      if (res.success) {
        ...
        const mapped = items.map((item: any) => {
          const firstName =
            item.creator?.first_name || item.contractor?.first_name || "";
          const lastName =
            item.creator?.last_name || item.contractor?.last_name || "";
          ...
          const location =
            [cityName, stateName, zipCode].filter(Boolean).join(", ") || "N/A";
          ...
          <div>
            ...
            {isAdmin && (
              <p className="text-[14px] md:text-[16px] font-normal text-[#708090] font-inter">
                Manage and review property additions submitted by Users.
              </p>
            )}
            {!isAdmin && (
              <p className="text-[14px] md:text-[16px] font-normal text-[#708090] font-inter">
                Review your added properties.
              </p>
            )}
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  <tr>
                    <td
                      colSpan={5}
                      className="py-20 text-center text-[#708090]"
                    >
                      <div className="flex flex-col items-center justify-center gap-3">
                        <Loader2 className="size-8 animate-spin text-[#1CA7A6]" />
                        <p className="text-sm font-semibold uppercase tracking-wider">
                  ...
                  <tr>
                    <td
                      colSpan={5}
                      className="py-16 text-center text-[#708090]"
                    >
                      <div className="flex flex-col items-center justify-center gap-2">
                        <MapPin className="size-8 opacity-40" />
        ...
        <AlertDialogContent className="sm:max-w-[425px] rounded-[20px] border-none shadow-[0px_4px_34px_rgba(31,42,68,0.1)] font-asap">
          <AlertDialogHeader className="space-y-3">
            ...
            <AlertDialogDescription className="text-sm text-gray-600 font-medium leading-relaxed font-inter">
              ...
              {confirmAction?.type === "APPROVE" ? "approve" : "reject"} the
              property addition request for{" "}
        ...
        <AlertDialogContent className="sm:max-w-[500px] rounded-[20px] border-none shadow-[0px_4px_34px_rgba(31,42,68,0.1)] font-asap overflow-y-auto max-h-[90vh]">
          ...
          <div className="space-y-4 py-4 text-[#1F2A44]">
            <div className="grid grid-cols-3 gap-2 items-start">
              <span className="text-[12px] font-bold uppercase tracking-wider text-[#708090]">
                Name
              </span>
            ...
            {selectedProperty?.raw?.square_foot && (
              <div className="grid grid-cols-3 gap-2 items-start">
                ...
                <span className="col-span-2 text-sm font-semibold">
                  {selectedProperty.raw.square_foot} sq ft
                </span>
              </div>
            )}
            <div className="grid grid-cols-3 gap-2 items-start">
              ...
              <div className="col-span-2 text-sm">
                <p className="font-semibold">
                  {selectedProperty?.contractorName}
                </p>
                <p className="text-[12px] text-[#708090]">
                  {selectedProperty?.contractorEmail}
                </p>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-2 items-start">
