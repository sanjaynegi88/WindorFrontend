...
import { createImageCategory, editImageCategory, getComponentTypes } from '@/lib/actions';
...
export function RoleFormDialog({ isOpen, onClose, state, onSuccess }: RoleFormDialogProps) {
  ...
  useEffect(() => {
    const fetchComponentTypes = async () => {
      ...
      setComponentTypes(res.data?.report_types || []);
    }
    fetchComponentTypes();
  }, []);
  ...
  const onSubmit = async (values: z.infer<typeof stateSchema>) => {
    ...
    try {
      if (state) {
        ...
        await editImageCategory(state.id, payload);
        ...
        await createImageCategory(payload);
