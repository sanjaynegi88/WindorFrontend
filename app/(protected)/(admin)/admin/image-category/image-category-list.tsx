...
export default function ImageCategoryListPage({ refreshTrigger, onSuccess }: { refreshTrigger: number, onSuccess: () => void }) {
  ...
  const fetchData = async (page: number = 1, limit: number = 10, name?: string) => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to load image categories');
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await deleteImageCategory(deleteId);
      if (!response.success) {
        ...
        cell: ({ row }) => {
          return (
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                <span className="font-semibold text-foreground text-sm leading-tight">
                  {toPascalCase(row.original.component_type)}
                </span>
              </div>
            </div>
          );
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent className="rounded-2xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the image category
          ...
          <AlertDialogFooter className="gap-2 sm:gap-0">
            ...
            >
              {isDeleting ? 'Deleting...' : 'Delete Image Category'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
