...
export function ServiceProvidedFormDialog({ isOpen, onClose, state, onSuccess }: StateFormDialogProps) {
  ...
  const onSubmit = async (values: z.infer<typeof stateSchema>) => {
    ...
    try {
      if (state) {
        const resposne = await editServiceProvided(state.id, payload);
        if (!resposne.success) {
        ...
        const response = await createServiceProvided(payload);
        if (!response.success) {
