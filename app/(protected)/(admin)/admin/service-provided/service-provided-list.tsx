...
import { getServiceProvided, deleteServiceProvided } from '@/lib/actions';
...
import { ServiceProvidedFormDialog } from './service-provided-form-dialog';
import { formatDate } from '@/lib/helpers';
import { toPascalCase } from '@/lib/utils';

export default function ServiceProvidedListPage({ refreshTrigger, onSuccess }: { refreshTrigger: number, onSuccess: () => void }) {
  ...
  const fetchData = async (page: number = 1, limit: number = 10, name?: string) => {
    setLoading(true);
    try {
      const response = await getServiceProvided(page, limit, name);
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await deleteServiceProvided(deleteId);
      if (!response.success) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="font-semibold text-foreground text-sm leading-tight">
                  {toPascalCase(row.original.service_name)}
                </span>
              </div>
            </div>
          );
  ...
  return (
    <>
      ...
      <ServiceProvidedFormDialog
        isOpen={isAddDialogOpen || !!editingService}
        onClose={() => {
