...
import { getRoleGroup } from "@/lib/user-role-fields";
  ...
  .superRefine((data, ctx) => {
    ...
    if (role === "PROPERTY_OWNER" || role === "REALTOR") {
      ...
      if (!data.cityOfficial?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "City official name is required",
          path: ["cityOfficial"],
        });
      }
      ...
      if (!data.cityPhone || !phoneRegex.test(data.cityPhone)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "City phone must be exactly 10 digits",
          path: ["cityPhone"],
        });
      }
      if (!data.title?.trim()) {
...
export default function EditUserPage({
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const formDetails =
          userInfo?.form_details || userInfo?.formDetails || {};
        ...
        const formData = {
          ...
          ownerDateStart: formDetails?.ownerDateStart
            ? formDetails.ownerDateStart.split("T")[0]
            : "",
          ...
          state_id: userInfo?.state_id
            ...
            : formDetails?.state_id
              ? String(formDetails.state_id)
              : "",
          city_id: userInfo?.city_id
            ? String(userInfo.city_id)
            : formDetails?.city_id
              ? String(formDetails.city_id)
              : "",
          zip: userInfo?.zip || formDetails?.zip || "",

          companyAddress: formDetails?.companyAddress || "",
          company_name:
            userInfo?.company_name || formDetails?.company_name || "",
          ...
          other_service: formDetails?.other_service || "",
  ...
  const executeSubmit = async (values: UserFormValues) => {
    ...
    if (values.sub_account !== undefined)
      formData.append("sub_account", String(values.sub_account));
    ...
    if (group === "property") {
      ...
      if (values.ownerDateStart)
        formData.append("ownerDateStart", values.ownerDateStart);
      if (values.ownerDateEnd)
        formData.append("ownerDateEnd", values.ownerDateEnd);
      ...
      if (values.other_service) {
        formData.append("other_service", values.other_service);
      }
      ...
      if (values.cityAddress)
        formData.append("cityAddress", values.cityAddress);
  ...
  const onSubmit = async (values: UserFormValues) => {
    ...
    if (
      group === "contractor" &&
      (!values.licenseNumber || values.licenseNumber.trim() === "")
    ) {
            ...
            <CardContent className="pt-8">
              <div className="flex flex-col items-center mb-10 group">
                <div className="relative">
                  <Avatar className="h-32 w-32 border-4 border-background">
                    <AvatarImage
                      src={
                        previewUrl ||
                        (userDataRaw?.profile?.profile_image_url
                          ? `${process.env.NEXT_PUBLIC_BASE_URL}${userDataRaw.profile.profile_image_url}`
                          : "")
                      }
                      className="object-cover"
                    />
                    <AvatarFallback className="text-3xl bg-primary/10 text-primary font-bold">
                      {userDataRaw?.first_name?.substring(0, 1).toUpperCase()}
                      {userDataRaw?.last_name?.substring(0, 1).toUpperCase() ||
                        ""}
                    </AvatarFallback>
                  </Avatar>
                  <button
                ...
                <div className="mt-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1 font-bold bg-muted px-3 py-1 rounded-full">
