...
export default function AuditLogsList() {
  ...
  const fetchLogs = async (pageNum: number = 1, reset: boolean = false) => {
    setLoading(true);
    try {
      const response = await getAuditLogs(pageNum, 50);
      if (reset || pageNum === 1) {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(log => {
      ...
      const matchesSearch =
        ...
        log.new_values?.email?.toLowerCase().includes(searchLower) ||
  ...
  const getDisplayName = (log: AuditLogEntry) => {
    const email = log.new_values?.email || 'System User';
        ...
        {Object.entries(groupedLogs).map(([date, dateLogs]) => (
          <div key={date} className="relative">
            ...
            <div className="space-y-4 pl-4 border-l-2 border-slate-100 dark:border-slate-800 ml-4">
              {dateLogs.map((log) => {
                const config = getActionInfo(log.action);
                  ...
                  >
                    <Card className={cn(
                      ...
                      <CardContent className="p-0">
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-3 flex items-center gap-4 text-[11px] text-slate-400 flex-wrap">
                              ...
                              {log.action !== 'LOGIN' && log.action !== 'LOGOUT' && log.action !== 'PASSWORD_FORGOT' && (
                        ...
                        <AnimatePresence>
                          {isExpanded && (!['LOGIN', 'LOGOUT', 'PASSWORD_FORGOT'].includes(log.action)) && (
                            <motion.div
