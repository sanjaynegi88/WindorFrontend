...
import {
  postProperty,
  postInstallation,
  postPropertyOwnerInstallations,
  getStates,
  getCities,
  getPropertyOwners,
...
function NewPropertyForm({ initialStep }: PropertyAddProps) {
  ...
  const [step, setStep] = useState<Step>(() => {
    if (typeof window !== "undefined") {
      ...
      if (
        flow === "add-installation" &&
        localStorage.getItem("current_property_id")
      ) {
        return "PROJECT";
      }
    }
  ...
  const [hasExistingReport, setHasExistingReport] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      if (flow === "add-installation") {
        const storedHasReport = localStorage.getItem(
  ...
  useEffect(() => {
    ...
    const loadPropertySummary = async () => {
      ...
      try {
        const storedHasReport =
          flow === "add-installation"
            ? localStorage.getItem("current_property_has_report")
            : null;
        ...
        const storedPropertyName =
          flow === "add-installation"
            ? localStorage.getItem("current_property_name")
            : null;
        if (storedPropertyName && isMounted) {
          setExistingPropertyName(storedPropertyName);
          setAddressData((prev) => ({
            ...prev,
            property_name: storedPropertyName,
          }));
        }
  ...
  const handleCreateProperty = async (
    ...
    try {
      ...
      if (!propertyId) {
        ...
        const reportFlag =
          propertyData?.has_report === true ||
          propertyData?.has_report === "true";
        const propertyName =
          propertyData?.property_name || addressData.property_name;
  ...
  const handleProjectCreate = (data: any) => {
    ...
    const projectId =
      data.projectData?.data?.id || data.projectData?.id || null;
    if (projectId) {
  ...
  const saveInstallation = async (
    ...
    if (!installationResult.success) {
      throw new Error(
        installationResult.message || `Failed to save ${type} installation`,
      );
    }

    const installationId =
      installationResult.data?.data?.id || installationResult.data?.id;
    if (!installationId)
      throw new Error(`Failed to save ${type} installation — no ID returned`);
  ...
  const handleStepSave = async (
    ...
    try {
      ...
      toast.error(
        error?.message || "Failed to save installation. Please try again.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="max-w-[1170px] mx-auto relative w-full">
            ...
            {step === "PROJECT" && (
              <CategorySelection
                ...
                onBack={() =>
                  initialPropertyId ? router.back() : setStep("ADDRESS")
                }
              />
            )}
            ...
            {step === "IMAGE_UPLOAD" && (
              <CategoryImageUpload
                ...
                onSave={async (photos) => {
                  ...
                  if (isOwner) {
                    try {
                      const filesToUpload = Object.values(photos).filter(
                        Boolean,
                      ) as File[];
                      for (const file of filesToUpload) {
                        const res = await uploadOwnerProjectImage(
                    ...
                    result = await uploadInstallationImages(
                      selectedCategory,
                      currentInstallationId,
              ...
              <div className="w-full max-w-[1170px] mx-auto space-y-[20px] md:space-y-[45px] animate-in fade-in slide-in-from-bottom-4 duration-500 font-asap px-[20px] md:px-0">
                <div className="text-center space-y-[10px] md:space-y-[15px]">
                  ...
                  <p className="text-[#708090] font-medium text-[14px] md:text-[18px] leading-relaxed max-w-[480px] mx-auto">
                    Your installation has been saved. You can add more projects
                    to this property, or go to Homepage.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-[12px] md:gap-[16px] pt-[10px] md:pt-[20px]">
                  {currentProjectId && (
                    <div>
                      {user?.role === "admin" ? (
                        <p className="text-[#708090] font-medium text-[14px] md:text-[18px] leading-relaxed  mx-auto">
                          Note: As an administrator, you can still edit or
                          delete this installation after confirmation.
                        </p>
                      ) : (
                        <p className="text-[#708090] font-medium text-[14px] md:text-[18px] leading-relaxed  mx-auto">
                          Note: After Confirmation you can not edit or delete
                          this installation.
                        </p>
                      )}
                      <button
