...
export async function proxy(request: NextRequest) {
  ...
  if (needsRefresh) {
    try {
      ...
      let userProfile = refreshData.user || refreshData.profile;
      if (!userProfile && refreshData.role) {
        userProfile = {
          role: refreshData.role,
          sub_account: refreshData.sub_account,
          has_membership: refreshData.has_membership,
        };
      }
  ...
  const requiresMembershipRedirect = !hasMembership && userRole !== 'admin' && userRole !== 'city_inspector' && !isSubUser;
  const defaultTarget = requiresMembershipRedirect ? '/plans' : '/dashboard';

  if (hasSession && isAuthOnlyRoute) {
    if (userRole === 'guest') {
      return getResponse('/select-role');
    }
    return getResponse(defaultTarget);
  }
...
export const config = {
  matcher: [
    '/((?!api|download|_next/static|_next/image|assets|favicon.ico|manifest.json|sw.js|.*\\.png$).*)',
  ],
};
