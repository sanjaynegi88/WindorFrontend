...
const roleColors: Record<string, string> = {
  ...
  CONTRACTOR:
    "bg-purple-500/10 text-purple-600 border-purple-200 dark:border-purple-500/20",
  MANUFACTURER:
    "bg-pink-500/10 text-pink-600 border-pink-200 dark:border-pink-500/20",
};
...
const roleIcons: Record<string, any> = {
  ...
  MANUFACTURER: <HardHat className="size-3 mr-1.5" />,
};
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      return (
        ...
        (item.role ?? item.roleEntity?.role_name ?? "")
          .toLowerCase()
