...
import { UserPlus, Loader2, ShieldCheck, EyeOff, Eye } from "lucide-react";
import {
  Card,
  CardContent,
  CardHeader,
...
export function AddSubUserForm({ route, roleLabel }: AddSubUserFormProps) {
  ...
  const onSubmit = async (values: StaffFormValues) => {
    ...
    if (checkoutUrl) {
      ...
      setCheckoutMessage(
        result.data?.message ||
          `Payment is required to add additional ${roleLabel.toLowerCase()} users. Would you like to proceed to checkout?`,
      );
    ...
    toast.success(
      `${roleLabel} staff "${values.firstName} ${values.lastName}" added successfully!`,
    );
    router.push(route);
  };
  ...
  const handleConfirmCheckout = () => {
    ...
    localStorage.setItem("pending_redirect_url", route);
            ...
            <CardHeader className="border-b justify-center bg-muted/20 py-4">
              <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <CardDescription>
                    Create a new {roleLabel.toLowerCase()} staff account.
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
