...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          'text-foreground/70 font-normal inline-flex h-full items-center gap-1.5 text-[0.8125rem] leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60',
          className,
        )}
      >
        {icon && icon}
  ...
  const headerButton = () => {
    return (
      <Button
        variant="ghost"
        className={cn(
          'text-foreground/70 rounded-md font-normal -ms-2 px-2 h-7 text-[0.8125rem] hover:bg-transparent data-[state=open]:bg-transparent hover:text-foreground/70 data-[state=open]:text-foreground/70 cursor-default',
          className,
        )}
      ...
      <div className="flex items-center h-full gap-1.5 justify-between">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-40" align="start">
            ...
            {filter && (column.getCanPin() || visibility) && (
                <DropdownMenuSeparator />
              )}

            {(filter) &&
              (column.getCanPin() || visibility) && (
                <DropdownMenuSeparator />
              )}
