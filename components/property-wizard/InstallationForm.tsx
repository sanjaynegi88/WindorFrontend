...
import { transformBrandsToOptions } from "@/lib/brand-utils";
...
const commonSchema = {
  ...
  contractorImages: z
    .array(z.string())
    .max(5, "Maximum 5 contractor images allowed")
    .optional(),
  ownerImages: z
    .array(z.string())
    .max(5, "Maximum 5 owner images allowed")
    .optional(),
...
export function InstallationForm({
  ...
  const [contractorImagePreviews, setContractorImagePreviews] = useState<
    string[]
  >([]);
  ...
  const brandOptions = useMemo(() => transformBrandsToOptions(brands), [brands]);
  const user = useUser();

  const isAdmin = user?.role === "admin";

  const [existingOwnerImages, setExistingOwnerImages] = useState<string[]>([]);
  const [existingContractorImages, setExistingContractorImages] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      ...
      try {
        ...
        const rawBrands = Array.isArray(response)
          ? response
          : response?.data || [];
        setBrands(rawBrands);
      } catch (error) {
        ...
        setCategoryPhotos((prev) => {
          ...
          cats.forEach((cat: any) => {
            const key = normalizeCategoryKey(
              cat.category_name || cat.display_name,
            );
  ...
  const [pendingAction, setPendingAction] = useState<"save" | "addImages">(
    "save",
  );
  ...
  const onSubmit = async (values: any) => {
    ...
    const allFiles = {
      contractorFiles: isEditMode ? [] : contractorFiles,
      ownerFiles,
      categoryFiles: categoryFileMap,
    };
          ...
          <div className="space-y-1 w-full">
            ...
            <CardTitle className="text-2xl flex items-center gap-2 capitalize">
              {type === "other_contractor"
                ? isAdmin
                  ? "Other Contractor Specifications"
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground flex flex-col gap-0.5">
                    <span>
                      {!isEditMode
                        ? "Upload a photo for each category."
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {imageCategories.map((cat) => {
                        ...
                        const existingImg = initialValues?.images?.find(
                          (img: any) =>
                            normalizeCategoryKey(img.image_category) === key,
                        );
                  ...
                  <div className="space-y-4">
                    ...
                    {existingOwnerImages.length > 0 && (
                      <div className="space-y-2">
                        <h4 className="text-sm font-medium text-muted-foreground">
                          Current Owner Images
                        </h4>
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                          {existingOwnerImages.map((url, index) => (
                            <div
                              key={`existing-owner-${index}`}
                              className="relative aspect-square rounded-xl overflow-hidden border bg-muted/20"
                            >
                              <Image
                                src={url}
                                alt={`Existing Owner ${index + 1}`}
                                fill
                                sizes="(max-width: 768px) 30vw, 15vw"
                                className="w-full h-full object-cover"
                              />
                              <div className="absolute top-1 left-1 px-1.5 py-0.5 bg-green-500 text-white text-xs rounded-md font-medium">
                                Current
                              </div>
                            </div>
                          ))}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {ownerImagePreviews.map((preview, index) => (
                        <div
                          key={index}
                          className="relative aspect-square rounded-xl overflow-hidden border bg-muted/20 group"
                        >
                          <Image
                            src={preview}
                            alt={`Owner ${index + 1}`}
